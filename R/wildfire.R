...
check_wildfire_vif <- function(
  ...
  if (save_csv) {
    ...
    # Note to include at the end of the file
    note <- c(
      "Note: VIF model validation results.",
      "Please refer to wildfire methods documentation (section 3.7) for interpretation guidelines."
    )
    #Append the note at the END of the file
    write(note, file = fpath, append = TRUE)
  }
}
...
calculate_qaic <- function(
  ...
  if (save_csv == TRUE) {
    ...
    # Note to include at the end of the file
    note <- c(
      "Note: QAIC model validation results.",
      "Please refer to wildfire methods documentation (section 3.7) for interpretation guidelines."
    )
...
save_wildfire_results <- function(
  ...
  write.csv(
    rr_results,
    file = file.path(output_folder_path, "RR_lag_estimates.csv"),
    row.names = FALSE
  )
...
#' file named "aggregated_AF_by_region.pdf" in the specified output_dir.
#'
...
#' @references
#' \enumerate{
#'  \item Brown A, Soutter E, Ingole V., Standards for Official Statistics on
#'  Climate-Health Interactions (SOSCHI): Wildfires: introduction. Zenodo; 2024.
#'  Available from: \url{https://zenodo.org/records/14052184}
#'  \item Hänninen R, Sofiev M, Uppstu A, Kouznetsov R.Daily surface
...
#' 	for aggregated data in environmental epidemiology: a tutorial.
#' 	Int J Epidemiol. 2024;53(2). Available from:
#' 	\doi{10.1093/ije/dyae020}
#' 	\item Wu Y, Li S, Guo Y. Space-Time-Stratified Case-Crossover Design in
#' 	Environmental Epidemiology Study. Heal Data Sci. 2021;
#'  Available from: \doi{10.34133/2021/9870798}
#' 	}
#'
#' @returns
#' \itemize{
